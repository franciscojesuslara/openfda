...
class OpenFDAparser():
    ...
    def data_company(self, drugs_1, list_1):
        for element in range(len(drugs_1["results"])):
            concha = ""
            try:
                # element["openfda"]["manufacturer_name"][0])
                concha = concha + element["openfda"]["manufacturer_name"][0]

            except KeyError:
                element = "Unknow"
                concha = concha + element
        return concha
        ...
        for i in range(len(drugs_1['results'])):
            ...
            try:
                if "openfda" in drugs_1["results"][i]:
                    concha = concha + drugs_1['results'][i]['openfda']["brand_name"][0]
            except KeyError:
                element = "Unknow"
                concha = concha + element
        return concha
    ...
    def extract_data_lcompanies(self, drugs_1, list_1):
        for i in range(len(drugs_1['results'])):
            try:
                if "openfda" in drugs_1['results'][i]:
                    lconcha = concha + drugs_1['results'][i]['openfda']["manufacturer_name"][0]
            except KeyError:
                element = "Unknow"
                concha = concha + element
        return concha

    def data_warning(self, drugs_1, list):
        for element in range(len(drugs_1["results"])):
            try:
    ...
    def do_GET(self):
        if self.path == "/":
            ...
            if "&" in self.path:
                ...
                limit = 10
                drug_limit = Client.active_component(drug, limit)
                patata = Parser.data_drug(drug_limit, list)
            file = HTML.html(patata)
            self.wfile.write(bytes(file, "utf8"))
            ...
            patata = Parser.data_company(rojo, list)
            file = HTML.html(patata)
            self.wfile.write(bytes(file, "utf8"))
            ...
            patata = Parser.data_company(rubi, list)
            file = HTML.html(patata)
            self.wfile.write(bytes(file, "utf8"))
            web_contents = file
            web_headers = "HTTP/1.1 200"
            ...
            patata = Parser.data_warning(plata, list)
            file = HTML.html(patata)
            self.wfile.write(bytes(file, "utf8"))
            web_contents = file
            web_headers = "HTTP/1.1 200"
